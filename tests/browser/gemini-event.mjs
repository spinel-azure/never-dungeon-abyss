...
await mkdir('artifacts/gemini-event',{recursive:true});
...
try {for(const [label,width,height] of [['pc',1280,900],['mobile',390,844]]) {
  ...
  for(const [choice,symbol] of [['confirm','sun'],['cancel','moon']]) {
    ...
    await page.screenshot({path:`artifacts/gemini-event/${label}-intro.png`});
    await page.evaluate(()=>gplayer.handleOverlayEventInput('confirm'));
    await page.waitForTimeout(1800);
    await page.screenshot({path:`artifacts/gemini-event/${label}-sisters.png`});
    ...
    await page.screenshot({path:`artifacts/gemini-event/${label}-${symbol}.png`});
    await page.evaluate(()=>gplayer.handleOverlayEventInput('confirm'));
    ...
    await page.screenshot({path:`artifacts/gemini-event/${label}-${symbol}-result.png`});
  }
