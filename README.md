...
現在は、探索エンジンへRPG本編の基盤を接続するMVP開発段階です。戦闘、町、職業、成長、装備、アイテム、デッキカード、ギルド依頼、宝箱罠、複数セーブの基本動作を実装済みです。

今後は、敵・装備・カード・アイテム・依頼の追加、商店や図書館などの施設サービス、転送陣、シナリオ、BGM、エンディングなどを順次実装予定です。
