...
await mkdir('artifacts/lion-opening',{recursive:true});
...
try {for(const [label,width,height] of [['pc',1280,900],['mobile',390,844],['small',320,720]]) {
  ...
  await page.waitForTimeout(600);await page.screenshot({path:`artifacts/lion-opening/${label}-intro.png`});
  ...
  await page.screenshot({path:`artifacts/lion-opening/${label}-form1.png`});
  ...
  for(const [hp,phase,file] of [[50001,2,'boss_27b.avif'],[25001,3,'boss_27c.avif']]) {
    ...
    if(phase===2) {
      ...
      assert.ok((await page.locator('#battleEnemyImage').getAttribute('src')).endsWith('boss_27.avif'));
      await page.screenshot({path:`artifacts/lion-opening/${label}-transition.png`});
    }
    ...
    await page.screenshot({path:`artifacts/lion-opening/${label}-form${phase}.png`});
  }
  // Real judgment produces the visible opportunity, then a guard expires it.
  await page.evaluate(async()=>{const {LION_JUDGMENT}=await import('/data/loewenkoenigin.js');lb.state().enemy.reservedEnemyAction=LION_JUDGMENT;void lb.use({type:'guard'});});
  await page.waitForFunction(()=>lb.idle());
  assert.equal(await page.evaluate(()=>lb.state().enemy.lionOpening),true);
  assert.ok((await page.locator('.battle-lion-opening').textContent()).includes('王者の隙'));
  assert.ok(await page.evaluate(()=>{
    const r=document.querySelector('.battle-lion-opening').getBoundingClientRect();
    const frame=document.querySelector('#battleScreen').getBoundingClientRect();
    const hp=document.querySelector('#battleBossHpMeter').getBoundingClientRect();
    return r.bottom<=frame.bottom && r.top>=hp.bottom && r.left>=frame.left && r.right<=frame.right;
  }),label+': opening badge fits below HP');
  await page.screenshot({path:`artifacts/lion-opening/${label}-opening.png`});
  await page.evaluate(()=>{void lb.use({type:'guard'});});await page.waitForFunction(()=>lb.idle());
  assert.equal(await page.evaluate(()=>Boolean(lb.state().enemy.lionOpening)),false);
  await page.evaluate(()=>{lb.state().enemy.lionOpening=true;lb.escape();});
  assert.equal(await page.evaluate(()=>Boolean(lb.state().enemy.lionOpening)),false);
  await page.evaluate(()=>lb.finish());
  ...
  await page.waitForTimeout(500);await page.screenshot({path:`artifacts/lion-opening/${label}-victory.png`});
  ...
  await page.screenshot({path:`artifacts/lion-opening/${label}-reward.png`});
  ...
  await page.screenshot({path:`artifacts/lion-opening/${label}-farewell.png`});
