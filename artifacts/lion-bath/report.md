...
- 隠しイベントは初回5%・50回目確定、目撃後0.5%・200回目確定。当選でカウントを0へ戻し、階移動・帰還・ロードでもリセット（非保存）。冒頭3秒はA/B入力不可、入力は繰り越さず押し直しが必要。Bで退室、Aで導入演出。再戦・再報酬なし。
...
検証：全1514件成功。PC1280×900・スマホ390×844・reduced-motionで確認。既存の噂→入室→戦闘→勝利→続きの噂も両幅で回帰確認。
