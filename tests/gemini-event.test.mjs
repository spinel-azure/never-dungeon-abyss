...
import {configurePlayer,state,startGeminiEvent,handleOverlayEventInput} from '../js/player.js';
...
test('Gemini dialogue uses A, choice uses A/B, and opening ignores repeated input',()=>{
  for(const [input,symbol] of [['confirm','sun'],['cancel','moon']]) {
    const c={};let callback,options,message;
    configurePlayer({say:s=>message=s,cancelAutoReturn:()=>{},playSe:()=>{},onStateChanged:()=>{},hideTreasure:()=>{},
      resolveGeminiChoice:choice=>resolveGeminiChoice(c,choice),
    ...
    assert.ok(message.includes(symbol==='sun'?'紋様の片割れ':'残念'));
