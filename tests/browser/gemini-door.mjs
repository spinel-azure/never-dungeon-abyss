...
await mkdir('artifacts/gemini-door',{recursive:true});
...
for(const depth of [22,31,44,48,59,73,1,18,23]){
...
await page.waitForTimeout(1000);await page.screenshot({path:`artifacts/gemini-door/b${depth}.png`});
}
assert.deepEqual(errors,[]);console.log('Gemini / Leo / ordinary purple doors rendered without page errors');
