...
test("battle skills hide passives and paginate as two columns of six", async () => {
  ...
  assert.match(styles, /\.skill-overlay\.is-battle-skills \.skill-overlay-list[\s\S]*repeat\(var\(--battle-item-rows, 6\),[\s\S]*repeat\(2,/);
...
test("battle items paginate as two columns of six without vertical scrolling", async () => {
  ...
  assert.match(source, /battleRows\(\) \* 2/);
  assert.match(source, /Math\.ceil\(overlay\.items\.length \/ battlePageSize\(\)\)/);
  assert.match(styles, /\.skill-overlay\.is-battle-items \.skill-overlay-list[\s\S]*repeat\(var\(--battle-item-rows, 6\),[\s\S]*repeat\(2,/);
