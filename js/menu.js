...
export function configureMenu(options) {
  ...
  menu.monsterCompendiumPanel = menu.root.querySelector('[data-menu-view="monsterCompendium"]');
  ...
  configureMonsterCompendium({
    root: menu.monsterCompendiumPanel,
    getCharacter: () => menu.getCharacter(),
    playSe: key => menu.playSe(key),
    onClose: () => {
      closeMonsterCompendium();
      menu.view = "dungeon";
      updateView();
    }
  });
...
export function completeGamepadBinding(action, button) {
  ...
  const conflict = Object.keys(bindings).find(key => key !== action && bindings[key] === button);
  bindings[action] = button;
  if (conflict) bindings[conflict] = previous;
  menu.gamepadBindings = bindings;
  menu.gamepadCaptureAction = "";
  updateOptionStates();
  persistSettings();
  menu.playSe("confirm");
  return true;
...
export function setDungeonColors({ wall, floor } = {}, { save = false } = {}) {
  if (["default", "stone", "red", "blue", "green", "yellow", "slate", "magic", "torture", "water", "crystal", "acacia", "light", "white", "black"].includes(wall)) menu.wallColor = wall;
  if (["default", "red", "blue", "green", "yellow", "slate", "magic", "torture", "water", "purple", "crystal", "acacia", "light", "white", "black"].includes(floor)) menu.floorColor = floor;
  applyWallColor();
  applyFloorColor();
  updateDebugStates();
...
export function openItemInventory() {
  if (menu.view !== "dungeon" && menu.view !== "commands") return false;
  openInventory();
  return true;
}
...
function handleAdventureRecords(action) {
  ...
  if (menu.adventureRecordsFocus !== "list") {
    if (action === "left" || action === "right") {
      menu.adventureRecordsFocus = menu.adventureRecordsFocus === "back" ? "next" : "back";
    } else if (action === "up" || action === "down") {
      if (entries.length) {
        const pageStart = menu.adventureRecordsPage * 10;
        menu.adventureRecordsCursor = action === "up" ? Math.min(entries.length - 1, pageStart + 9) : pageStart;
        menu.adventureRecordsFocus = "list";
      }
    } else if (action === "confirm") {
      if (menu.adventureRecordsFocus === "back") {
        if (menu.adventureRecordsPage === 0) { menu.view = "dungeon"; updateView(); return; }
        menu.adventureRecordsPage -= 1;
      } else if (menu.adventureRecordsPage < pages - 1) menu.adventureRecordsPage += 1;
  ...
  if (action === "up" || action === "down") {
    ...
    const pageStart = menu.adventureRecordsPage * 10;
    const pageEnd = Math.min(entries.length - 1, pageStart + 9);
    if (action === "up" && menu.adventureRecordsCursor === pageStart) menu.adventureRecordsFocus = "back";
    else if (action === "down" && menu.adventureRecordsCursor === pageEnd) menu.adventureRecordsFocus = "back";
    else menu.adventureRecordsCursor += action === "down" ? 1 : -1;
    renderAdventureRecords();
    return;
  }
}
...
function bindAdventureRecords() {
  ...
  panel.querySelector('[data-adventure-records-nav="next"]').addEventListener("click", () => {
    menu.adventureRecordsFocus = "next";
    handleAdventureRecords("confirm");
  });
}
...
function renderAdventureRecords() {
  ...
  panel.querySelector("[data-adventure-records-list]").replaceChildren(...entries.slice(menu.adventureRecordsPage * 10, menu.adventureRecordsPage * 10 + 10).map((entry, offset) => {
    const index = menu.adventureRecordsPage * 10 + offset;
    const button = document.createElement("button");
    button.type = "button";
    button.className = "inventory-entry";
  ...
  const selected = entries[menu.adventureRecordsCursor];
  panel.querySelector("[data-adventure-records-description]").textContent = selected?.description || "記録はありません。";
  panel.querySelector("[data-adventure-records-page]").textContent = `${menu.adventureRecordsPage + 1}/${pages}`;
  const back = panel.querySelector('[data-adventure-records-nav="back"]');
  const next = panel.querySelector('[data-adventure-records-nav="next"]');
  back.textContent = menu.adventureRecordsPage > 0 ? "PREV" : "BACK";
...
function handleOptions(action) {
  if (menu.gamepadCaptureAction) {
    if (action === "cancel") { menu.gamepadCaptureAction = ""; updateOptionStates(); }
    return;
  }
...
function executeOptionNav(key) { menu.gamepadCaptureAction = ""; if (key === "back") { if (menu.optionPage === 0) closeOptions(); else setOptionPage(menu.optionPage - 1); } else if (menu.optionPage < menu.optionPages.length - 1) setOptionPage(menu.optionPage + 1); else closeOptions(); }
...
function updateView() {
  ...
  document.body.classList.toggle("monster-compendium-open", menu.view === "monsterCompendium");
  document.body.classList.toggle("card-gallery-open", menu.view === "cardGallery");
  menu.root.hidden = !screenOpen; menu.statusPanel.hidden = menu.view !== "status"; menu.deckPanel.hidden = menu.view !== "deck"; menu.inventoryPanel.hidden = menu.view !== "inventory"; menu.questHistoryPanel.hidden = menu.view !== "questHistory"; menu.rumorHistoryPanel.hidden = menu.view !== "rumorHistory"; menu.adventureRecordsPanel.hidden = menu.view !== "adventureRecords"; menu.monsterCompendiumPanel.hidden = menu.view !== "monsterCompendium"; menu.cardGalleryPanel.hidden = menu.view !== "cardGallery"; menu.savePanel.hidden = menu.view !== "save"; menu.optionsPanel.hidden = menu.view !== "options"; menu.debugPanel.hidden = menu.view !== "debug";
...
function updateOptionStates() {
  ...
  for (const action of Object.keys(gamepadDefaults)) {
    ...
    if (state) state.textContent = menu.gamepadCaptureAction === action ? "PRESS BUTTON..." : `BUTTON ${button}`;
  }
}
  ...
  try {
    ...
    if (saved.gamepadBindings && typeof saved.gamepadBindings === "object") {
      ...
      if (values.every(value => Number.isInteger(value) && value >= 0 && value <= 3) && new Set(values).size === values.length) {
        const remaining = [0, 1, 2, 3].find(value => !values.includes(value));
        const savedItems = Number(saved.gamepadBindings.items);
        menu.gamepadBindings = { confirm: values[0], cancel: values[1], minimap: values[2], items: Number.isInteger(savedItems) && savedItems >= 0 && savedItems <= 3 && !values.includes(savedItems) ? savedItems : remaining };
      }
    }
