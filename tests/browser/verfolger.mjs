...
const out=path.resolve('artifacts/verfolger');await mkdir(out,{recursive:true});
...
try{for(const [label,width,height,touch] of [['pc',1280,900,false],['mobile',390,844,true]]){
 ...
 await page.route('**/js/battle.js',r=>r.fulfill({contentType:'text/javascript',body:battle+`\nwindow.vbattle={state:()=>structuredClone(battleUi.battle),idle:()=>!battleUi.presenting,escape:attemptEscape,snapshot:()=>createBattleCompletionSnapshot(battleUi.battle),weaken(){battleUi.battle.enemy.hp=1;battleUi.battle.enemy.stats.agi=0;battleUi.battle.player.hp=battleUi.battle.player.maxHp;}};`}));
 await page.goto(origin);await page.waitForFunction(()=>window.vqa);await page.evaluate(()=>{vqa.setup();vqa.near();});
 ...
 assert.equal((await page.evaluate(()=>vBgm())).key,'dungeon');assert.equal((await page.evaluate(()=>vqa.map())).status,'active');
 await page.evaluate(()=>{vqa.contact();vqa.confirm();});await page.waitForFunction(()=>vbattle.state()?.enemy?.id==='verfolger'&&vbattle.idle());
 ...
 await page.evaluate(snapshot=>vqa.replay(snapshot),snapshot);assert.deepEqual(await page.evaluate(()=>vqa.state()),c);
 ...
 await page.evaluate(()=>vqa.regenerate(91));assert.equal((await page.evaluate(()=>vqa.map())).status,'active');
 await page.evaluate(()=>vqa.regenerate(99));assert.equal(await page.evaluate(()=>vqa.map()),null);
 for(const rumorId of ['rumor_013','rumor_014'])for(const defeated of [false,true]){
  ...
  for(let index=0;index<(defeated?4:3);index++){
   ...
   const flag='tavern_'+rumorId+'_'+(defeated?(rumorId==='rumor_013'?'defeated':'lichtbringer'):'base')+'_read';assert.equal(Boolean((await page.evaluate(()=>vqa.state())).eventFlags[flag]),false);
  ...
  const c=await page.evaluate(()=>vqa.state());assert.equal(c.eventFlags['tavern_'+rumorId+'_'+(defeated?(rumorId==='rumor_013'?'defeated':'lichtbringer'):'base')+'_read'],true);
  await page.evaluate(()=>vTown.service('past-rumors'));assert.ok((await page.locator('body').innerText()).includes(rumorId==='rumor_013'?'暗闇から忍び寄る追跡者の噂':'「光もたらすもの」の噂'));
 }
 assert.deepEqual(errors,[]);results.push({label,draw,bgm:'battle-of-galfer.mp3',escape:true,victory:true,save:true,noRespawn:true,rumorBase:true,rumorFollowup:true,rumorHistory:true,errors});await context.close();
