...
for (const [jobId, hp, sp] of [["warrior",999,650],["thief",850,750],["priest",750,850],["mage",650,999]]) { assert.equal(getLevelGrowth(jobId,1).hp,JOBS[jobId].hp); assert.equal(getLevelGrowth(jobId,1).sp,JOBS[jobId].sp); assert.equal(getLevelGrowth(jobId,197).hp,hp); assert.equal(getLevelGrowth(jobId,197).sp,sp); }
