...
const hook=`window.spellQa={setup(id){
 ...
 character=createInitialCharacter({name:'Spell QA',job:'priest'});character.skillIds=[id];character.sp=character.maxSp=200;character.hp=character.maxHp=9999;character.int=30;character.agi=99;character.playerCharge={value:100,cooldown:0};character.deckTutorialSeen=true;
...
await mkdir('artifacts/goddess',{recursive:true});const browser=await chromium.launch({channel:'msedge',headless:true});const results=[];
try{
for(const [label,width,height,touch] of [['pc',1280,900,false],['mobile',390,844,true]]){
const context=await browser.newContext({viewport:{width,height},isMobile:touch,hasTouch:touch});const page=await context.newPage();await page.addInitScript(()=>navigator.getGamepads=()=>[]);const errors=[];page.on('pageerror',e=>errors.push(e.message));
...
for(const id of ['call_goddess_name']){
...
await page.waitForFunction(()=>!document.querySelector('#battleSkillEffectCanvas').hidden);await page.waitForTimeout(2200);

await page.screenshot({path:`artifacts/goddess/${label}-${id}.png`});
await page.waitForFunction(()=>window.castDone,{},{timeout:45000});
const played=await page.evaluate(()=>window.playedEffects);assert.equal(played.length,id==='apocalypse'?2:1);assert.ok(played.every(e=>e.id==='goddess'&&e.damage>0&&e.popup===String(e.damage)));
...
await page.evaluate(()=>{spellQa.setup('call_goddess_name');window.castDone=false;battleQa.cast('call_goddess_name').then(()=>window.castDone=true);});
...
await writeFile('artifacts/goddess/results.json',JSON.stringify(results,null,2));console.log(results);
