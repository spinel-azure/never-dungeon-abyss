...
window.bathAudioActive=()=>audio.desiredLoops.has('lionBathWater');`}));
 ...
 await page.waitForTimeout(50);assert.equal(await page.evaluate(()=>bathAudioActive()),true);await page.evaluate(()=>lp.handleOverlayEventInput('cancel'));await page.waitForFunction(()=>!lp.state.anim);assert.equal(await page.locator('.lion-bath-layer').count(),0);assert.equal(await page.evaluate(()=>bathAudioActive()),false);
 await page.evaluate(()=>{startRare();lp.handleOverlayEventInput('confirm');});await page.waitForFunction(()=>lp.state.overlayEvent?.phase==='bathReveal');
