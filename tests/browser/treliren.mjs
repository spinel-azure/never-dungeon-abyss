...
await mkdir('artifacts/treliren',{recursive:true});
...
try{for(const [label,width,height,touch]of[['pc',1280,900,false],['mobile',390,844,true],['small',320,720,true]]){
 ...
 while((await page.evaluate(()=>tQa.status())).event==='trelirenTalk'&&steps++<45){
  ...
  if(steps===1)await page.screenshot({path:`artifacts/treliren/${label}-first.png`});
 ...
 await page.screenshot({path:`artifacts/treliren/${label}-repeat.png`});
...
}await writeFile('artifacts/treliren/results.json',JSON.stringify(results,null,2));console.log(results);}finally{await browser.close();}
