...
test("B95F reserves the Lichtbringer pickup for request 033's event room", () => {
  assert.equal(getQuestEventForDepth(95, { activeQuestIds: [], eventFlags: {} }), null);
  const event = getSpecialRoomDefinition(95).content;
  assert.equal(event.type, "keyItemPickup");
  assert.equal(event.questId, "guild_033");
  assert.equal(event.keyItemId, "lichtbringer");
  assert.equal(event.flag, "lichtbringer_b95f_found");
...
test("Lichtbringer restores normal torch behavior instead of permanent illumination", async () => {
  ...
  assert.match(main, /if \(keyItem\.id === "lichtbringer"\)[\s\S]*state\.torchFuel = 100/);
  assert.match(main, /isForcedTorchZeroFloor\(currentDepth\) && !lightbringerOwned/);
  assert.match(main, /state\.lightbringerActive = false/);
